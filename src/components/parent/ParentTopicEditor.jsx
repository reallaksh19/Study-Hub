...
export function ParentTopicEditor({ subjectId, topicFolder, subjects = [], topics = [], onDirtyChange }) {
  ...
  const saveTopicMeta = async (updated) => {
    ...
    catch (err) { toast.show(`Failed to save topic: ${err.message}`, { variant: 'error' }); }
  ...
  const handleAddPage = async () => {
    ...
    try {
      ...
      toast.show('Failed to add page.', { variant: 'error' });
    }
  };
  ...
  const handleDeleteTopic = async () => {
    const ok = await confirm({
      title: `Delete topic "${topicData.title}"?`,
      body: 'This permanently deletes the topic and every page inside it. This cannot be undone.',
      confirmLabel: 'Delete topic',
      variant: 'danger',
      typeToConfirm: topicData.title,
    });
    if (!ok) return;
    try {
      ...
      toast.show(`Failed to delete topic: ${err.message}`, { variant: 'error' });
    }
  };
